...
# Screen 1.2: Enter Reset OTP (Forgot Password)
...
| Action | Behaviour |
| --- | --- |
| **Verify OTP** | If OTP matches → shows popup: "Verification successful" → navigates to Reset Password (Screen 1.3). If OTP mismatch → shows popup: "Invalid OTP. Please try again." |
| **Resend OTP** | Sends new OTP → toast: "OTP resent successfully." If failed → toast: "Failed to resend OTP. Please try again." |
...
| Action | Behaviour |
| --- | --- |
| **Reset Password** | On success → toast: "Password reset successfully." → navigates to Login (Screen 1) |
| **Back to Sign In** | Navigates to Login (Screen 1) |
...
| Calendar | 📅 | Calendar | Screen 5: Calendar Page (Tabs: Attendance Calendar \| Leaves) | — |
| Petty Cash | 💰 | Petty Cash | Screen 20: Petty Cash — My Requests | Count of Draft / Returned requests |
| Settings | ⚙️ | Settings | Screen 21: Settings | — |

**Note:**
- **Profile is not part of bottom navigation.** Profile opens from the **header profile icon** (Screen 2 header) and navigates to Screen 7.
...
# Screen 5: Calendar Page (Tabs: Attendance Calendar | Leaves)

**Source Reference:** Module 25 – HRM (25.3 Attendance Calendar), Module 21 – Task Management (21.1 Calendar Dashboard)
**Purpose:** Visual calendar showing the technician's attendance records and task schedule, with a second tab for Leaves (history + apply).
...
| Tab Switcher | Tabs | **Calendar View** (default) and **Leaves**. Switching tabs preserves state per tab |
...
# Screen 5.1: Leaves (Tab Content)

**Source Reference:** Module 25 – HRM (25.4 Leave Application, 25.5 Leave Requests)
**Purpose:** View leave history and apply for new leave requests, inside the Calendar screen as a second tab.
...
│  │  🤒 Sick Leave                        │    │
│  │  24 Mar 2026 (1 day)                  │    │
│  │  Leave Period: 24 Mar 2026 → 24 Mar 2026 │  │
...
| --- | --- | --- |
| Apply Leave | Tap "+ APPLY LEAVE" button | Opens Apply Leave Form (Screen 5.1.1) |

---

# Screen 5.1.1: Apply Leave Form
...
## Error Messages (Leave)

| Condition | Message | Behaviour |
| --- | --- | --- |
| Insufficient leave balance | "Insufficient leave balance. You have X days remaining." | Inline under date fields + blocks submission |
| Balance not available / zero for type | "You don't have leave balance for selected leave type." | Shows error toast + blocks submission |

## Actions

| Action | System Behaviour |
| --- | --- |
| **Submit Request** | Validates form → Creates leave request with Status = Pending → Appears in Module 25 Tab 2 (Leave Requests) for HR/Manager approval → Shows success toast: "Leave request submitted successfully." → Returns to Leaves tab (Screen 5.1) |
| **Cancel** | Discards form and returns to Leaves tab (Screen 5.1) |
...
| --- | --- | --- |
| **Start Travel** | Visible when Status = Pending and user has active Punch In | Starts travel tracking (Module 22) for this task → disables button immediately (prevents double tap) → shows toast: "Travel started." |
| **Start Task** | Visible only when backend returns `travel_started = true` AND Status = Pending AND current time is within ±30 min of scheduled start | Initiates task execution flow → Opens Selfie Capture (Screen 10) |
...
| Travel First | Task execution can start only after Travel is started for that task (`travel_started = true`) |
| Start Travel Disable | Once "Start Travel" is tapped, the button becomes disabled until backend confirms travel started (or error is returned) |
...
| Optional Step | User can skip this step. If skipped, the report still generates but photos section remains empty |
...
| **Skip for now** | Tap link/button | Skips uploading → Navigates to After Service Photos (Screen 12) |
| **Continue** | Tap button | Navigates to After Service Photos (Screen 12) |
...
| **Capture Photo** | Tap + or empty slot | Opens camera → captures photo → adds to grid |
| **Delete Photos** | Tap 🗑 Delete Photos button | Enters selection mode (see Delete Flow below) |
| **Skip for now** | Tap link/button | Skips uploading → Navigates to Service Execution Form (Screen 13) |
| **Continue** | Tap button | Navigates to Service Execution Form (Screen 13) |
...
|small image of product / checmical|Image|No|Display image of product / checmical|
...
| Action | Behaviour |
| --- | --- |
| **Continue** | Validates all 3 observation sections → Navigates to After Task Photos (Screen 15) |
| **Back** | Returns to Service Execution Form (Screen 13) — data is saved in draft |

---

# Screen 15: After Task Image Upload (Optional)

**Source Reference:** Module 21 – Task Completion (Photo Evidence)
**Purpose:** Allow the technician to attach **after-task** photos as supporting evidence (optional, up to 5). This screen replaces the previous "Technician Info & Notes" screen.
...
```
┌──────────────────────────────────────────────┐
│  [← Back]      AFTER TASK PHOTOS             │
│  Task: TASK-2026-0201                        │
│                                              │
│  Upload photos after completing service.     │
│  (Optional, Max 5)                           │
│                                              │
│  ┌──────┐  ┌──────┐  ┌──────┐  ┌──────┐     │
│  │ 📸   │  │ 📸   │  │  +   │  │      │     │
...
| Element / Field | Type | Description |
| --- | --- | --- |
| Page Header | Header | "AFTER TASK PHOTOS" with back button |
| Photo Thumbnails | Image Grid | Preview thumbnails of captured photos |
| Add Button (+) | Camera Trigger | Opens camera to capture additional photos |
| Photo Counter | Display | "X / 5" counter |
| Delete Photos Button | Button | Uses the same multi-step delete flow as Screens 11–12 |
| Skip for now | Link/Button | Skips and continues |
...
| Maximum 5 Photos | Cannot upload more than 5 photos |
| Camera Only | Photos must be captured live (no gallery upload) |
| File Format | JPG, PNG only (Max 5MB per photo) |
| GPS Embedded | Each photo includes GPS metadata |
...
| --- | --- | --- |
| **Capture Photo** | Tap + or empty slot | Opens camera → captures photo → adds to grid |
| **Delete Photos** | Tap 🗑 Delete Photos | Enters selection mode and deletes selected photos |
| **Skip for now** | Tap link/button | Navigates to Customer Verification (Screen 16) |
| **Continue** | Tap button | Navigates to Customer Verification (Screen 16) |
| **Back** | Tap ← | Returns to Observations (Screen 14) — data saved in draft |
...
Removed as per updated mobile flow. Submission eligibility is enforced by backend validations.
...
| **Share (Email)** | Sends email to the **SO-connected contact / Contract contact / Billing contact** (priority order from backend) with attachments: (1) Service Report PDF, (2) Excel of previous service logs (Customer module service history export), (3) Previous service details in full (as HTML/PDF), (4) Invoice PDF for this service (Module 28). Shows toast: "Service report emailed successfully." |
